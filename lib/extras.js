(function ()
{
  ...
  if (typeof(window) !== 'undefined') {
    ...
    PriorityQueue = require("./utils/PriorityQueue");
  } else if (typeof(load) !== 'undefined') {
    // jsc
    var Graph = load('lib/graph.js').Graph;
    PriorityQueue = load("./utils/PriorityQueue");
  }
  ...
  Graph.prototype.dijkstra = function (startNode) {
    let distances = {};
    let prev = {};
    ...
    while (!pq.isEmpty()) {
      ...
      let adjacents = this.adj(currNode)

      Object.keys(adjacents).forEach(v => {
        let alt = distances[currNode].weight + this.get(currNode, v);
        let path = distances[currNode].path.concat([v])

        if (alt < distances[v].weight) {
          distances[v] = {
            weight: alt,
            path
          };
          ...
          prev[v] = currNode;
          pq.enqueue(v, distances[v].weight);
        }
      });
    }
